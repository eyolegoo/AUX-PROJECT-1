# AUX-PROJECT-1
SHELL SCRIPTING TO ONBOARD NEW USERS TO A SERVER

## AUX-PROJECT-1

***SHELL SCRIPTING TO ONBOARD NEW USERS TO A SERVER***

- You have been learning about Linux for some time, and it is time to start getting a feel of how to automate some work using Shell Scripts.

- In this project, I onboarded 20 new Linux users onto a server. I **Create a shell script** that reads a csv file that contains the first name of the users to be onboarded.

- I created the project folder called **Shell** `mkdir Shell`.

- I moved into the Shell folder `cd Shell`.

- I created a csv file name **names.csv** `touch names.csv`.

- I opened the names.csv file using `vi names.csv`

```
TenStegen
Alba
Roberto
Kounde
Pique
Gavi
Pedri
Dembele
Messi
```

- The script I created reads the CSV file, created each user on the server, and added to an existing group called **developers** (I manually created this group ahead).

- I ensured that my script will first check for the existence of the user on the system, before it will attempt to create that it.

- I ensured that the user that is being created also has a default home folder.

- I ensured that each user has a **.ssh folder** within its **HOME folder**. If it does not exist, then it created it.

- For each user’s SSH configuration, I created an **authorized_keys** file and I ensured it has the **public key** of your current user.

- Before Deploying my script, I updated my current user with the correct **public key and private key**.

- In the current home directory, I changed directory to .ssh folder `cd .ssh`

- I created a file for the **public key** `touch id_rsa.pub` and I opened the file using `vi id_rsa.pub`

- In the emptied file, I copied and pasted the public key generated when creating the instance.

```
ssh-rsa 
```
- I created a file for the **private key** `touch id_rsa` and I opened the file using `vi id_rsa`

- In the emptied file, I copied and pasted the private key generated when creating the instance.

```
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
```

- I tested a few of the users randomly, and ensure that I was able to connect to the server using the **private key and the public key**.

[Onboarding New Users to Server](https://drive.google.com/file/d/1g7rWvz3vIKZd-YVtmWfhqaMW6kaWmD6-/view?usp=share_link)
